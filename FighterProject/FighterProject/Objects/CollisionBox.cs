...
namespace FighterProject.Library {
    ...
    class CollisionBox {
        ...
        /// <summary>
        /// Scale the collision box width and height.
        /// </summary>
        /// <param name="scale">The scale.</param>
        /// <returns>The scaled collision box.</returns>
        public CollisionBox ScaleSize(float scale) {
            return new CollisionBox(Offset.X, Offset.Y, Width * (int)scale, Height * (int)scale);
        }

        /// <summary>
        /// Scale the collision box offset.
        /// </summary>
        /// <param name="scale">The scale.</param>
        /// <returns>The scaled collision box.</returns>
        public CollisionBox ScaleOffset(float scale) {
            return new CollisionBox(Offset.X * scale, Offset.Y * scale, Width, Height);
        }
        ...
        public static bool DidCollide(Vector2 atkDrawPos, Vector2 defDrawPos, CollisionBox atkBox, CollisionBox defBox, float atkScale, float defScale) {
            ...
            float defRight = defDrawPos.X + defBox.ScaleSize(defScale).Width + defBox.Offset.X;
            float defBottom = defDrawPos.Y + defBox.ScaleSize(defScale).Height + defBox.Offset.Y;
